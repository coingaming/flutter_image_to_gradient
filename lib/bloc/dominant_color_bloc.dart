...
class DominantColorBloc extends Bloc<DominantColorEvent, DominantColorState> {
  ...
  Future<void> _mapLoadDominantColorToState(
    ...
    emit(state.copyWith(
      status: DominantColorStatus.loading,
      imagePath: event.imagePath,
    ));

    try {
      final colorModel = await _getDominantColor(event.imagePath);
      emit(state.copyWith(
        status: DominantColorStatus.loaded,
      ...
      debugPrint("Error: $e");
      emit(state.copyWith(
        status: DominantColorStatus.error,
  ...
  Future<DominantColorModel> _getDominantColor(String imagePath) async {
    try {
      ...
      List<int> sortedColors = colorFrequency.keys.toList()
        ..sort((a, b) =>
            (colorFrequency[b] ?? 0).compareTo(colorFrequency[a] ?? 0));
